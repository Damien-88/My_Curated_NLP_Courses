...
Lemmatization Engines utilze Part-of_Speech tags to determine dictionary base forms (lemmas), and evaluate performance 
...
uses a full linguistic dictionary and morphoogical analysis to accurately return the true, grammatically correct base
...
* Sentence B: *"Time **flies** when you are coding."* $\rightarrow$ Here, "flies" is a **Verb** (third-person singular). 
  The correct lemma is **"fly"**.
* Sentence C: *"He wears **flies** clothing."* $\rightarrow$ In slang/adjective usage, its underlying structure changes
  entirely.
...
|  **Output Validity** |Often returns non-words (`destabil` / `alumnu`).|   Always returns a real dictionary word.   |
